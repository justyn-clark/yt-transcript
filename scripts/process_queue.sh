...
QUEUE_DIR="${YT_TRANSCRIPT_QUEUE_DIR:-/Users/jcnagent/Agent/Ops/youtube-transcribe}"
...
NOTES_DIR="${YT_TRANSCRIPT_NOTES_DIR:-/Users/jcnagent/Agent/Vault}"
NOTES_SUBDIR="${YT_TRANSCRIPT_NOTES_SUBDIR:-50 - Content & Ideas/Transcripts/YouTube}"
...
if [[ ! -x "$CLI_BIN" ]]; then
  echo "yt-transcript CLI not found at $CLI_BIN. Run: $PYTHON_BIN -m pip install -e '.[dev]'" >&2
  exit 2
fi
...
while IFS= read -r url; do
  ...
  out_file="$LOG_DIR/$run_id.$(printf '%s' "$url" | shasum -a 256 | cut -c1-12).json"
  err_file="$LOG_DIR/$run_id.$(printf '%s' "$url" | shasum -a 256 | cut -c1-12).err"
...
fields=[data.get('source_id',''), data.get('retrieval_method',''), str(data.get('segment_count','')), data.get('notes_path',''), data.get('title','')]
